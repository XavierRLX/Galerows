...
export function Top10ResultScreen() {
  ...
  useEffect(() => {
    ...
    const match = session.config.mode === 'teams'
      ? createTeamGaleraMatchResult({ matchId: session.id, gameId: 'top-10', gameName: 'Top 10', finishedAt: session.updatedAt, participants: session.participants, teams: session.teams, scores: session.scores })
      : createIndividualGaleraMatchResult({ matchId: session.id, gameId: 'top-10', gameName: 'Top 10', finishedAt: session.updatedAt, entities: session.participants, scores: session.scores })
  ...
  return <div className="min-h-dvh pb-10"><AppReviewCheckpoint matchId={session.id} /><Header title="Fim de jogo" /><section className="px-5 py-8 text-center"><Trophy className="mx-auto text-red-300" size={64} /><h1 className="mt-4 text-3xl font-black">{winners.length > 1 ? 'Temos um empate!' : `${winners[0]?.name} venceu!`}</h1><p className="mt-2 text-slate-400">Vence quem somou mais pontos nas listas.</p>
