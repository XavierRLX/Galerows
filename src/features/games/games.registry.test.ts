...
describe('gamesRegistry', () => {
  it('keeps implemented games available', () => {
    ...
    expect(gamesRegistry.filter((game) => game.status === 'available').map((game) => game.id)).toEqual(['nem-ferrando', 'impostor-da-palavra', 'taboo', 'quem-sou-eu', 'adedonha', 'mimica', 'top-10'])
  })
