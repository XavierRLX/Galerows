...
export function NemFerrandoResultScreen() {
  ...
  useEffect(() => { if (session?.phase === 'finished') void recordGaleraMatchResult(createIndividualGaleraMatchResult({ matchId: session.id, gameId: 'nem-ferrando', gameName: 'Nem Ferrando', finishedAt: session.updatedAt, entities: session.participants, scores: session.scores, lowerScoreWins: true })) }, [session])
  ...
  return <div className="min-h-dvh pb-10"><AppReviewCheckpoint matchId={session.id} /><Header title="Fim de jogo" /><section className="px-5 py-8 text-center"><Trophy className="mx-auto text-yellow-300" size={64} /><h1 className="mt-4 text-3xl font-black">{winners.length > 1 ? 'Temos um empate!' : `${winners[0]?.name} venceu!`}</h1><p className="mt-2 text-slate-400">Vence quem terminou com menos Ferros.</p>
