...
export function AdedonhaPlayScreen() {
  ...
  return <div className="min-h-dvh pb-10"><Header backTo="/games/adedonha" title="Adedonha" action={<Button size="icon" variant="secondary" onClick={() => setShareOpen(true)}><QrCode size={18} /></Button>} /><section className="px-5 py-6">
    ...
    {session.phase === 'finished' ? <section className="mt-5"><Card className="p-5 text-center"><h2 className="text-2xl font-black">Partida finalizada</h2><p className="mt-2 text-sm text-slate-400">{session.rounds.length} rodadas salvas</p><div className="mt-5 rounded-3xl bg-yellow-400 p-6 text-slate-950"><p className="text-sm font-black uppercase tracking-wider">Pontuação final</p><p className="text-5xl font-black">{matchTotal} pts</p></div></Card><RoundsHistory rounds={session.rounds} /><div className="mt-5 grid gap-3"><Button className="bg-yellow-400 text-slate-950 hover:bg-yellow-300" size="lg" onClick={async () => { await discard(); navigate('/games/adedonha/setup') }}><Home size={18} />Nova partida</Button></div></section> : null}
