...
export function AdedonhaQrScanner({ onClose, onScan }: { onClose: () => void; onScan: (value: string) => void }) {
  ...
  useEffect(() => {
    ...
    navigator.mediaDevices?.getUserMedia({ video: { facingMode: { ideal: 'environment' } }, audio: false })
      ...
      .catch(() => setError('Não consegui abrir a câmera. Confira a permissão do app e tente novamente.'))
  ...
  return <div className="fixed inset-0 z-50 bg-slate-950 text-white">
    <div className="flex min-h-dvh flex-col">
      <div className="flex items-center justify-between gap-3 border-b border-white/10 px-5 py-4"><div className="flex items-center gap-2 font-black"><Camera className="text-yellow-300" size={20} />Escanear QR</div><Button aria-label="Fechar scanner" size="icon" variant="ghost" onClick={onClose}><X size={20} /></Button></div>
      <section className="flex flex-1 flex-col justify-center px-5 py-6 text-center">
        ...
        <p className="mx-auto mt-5 max-w-sm text-sm leading-6 text-slate-300">Aponte a câmera para o QR Code dos temas. A importação acontece automaticamente.</p>
