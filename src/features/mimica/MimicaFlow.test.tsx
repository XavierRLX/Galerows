...
describe('Mimica complete flow', () => {
  ...
  it('creates guests, scores actor and guesser, and reaches the result', async () => {
    ...
    await user.click(await screen.findByRole('button', { name: /trocar cena/i }))
    await user.click(await screen.findByRole('button', { name: /escovar os dentes/i }))
    await user.click(await screen.findByRole('button', { name: /começar mímica/i }))
    await user.click(await screen.findByRole('button', { name: /encerrar e registrar resultado/i }))
