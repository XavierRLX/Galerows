import { Clock3, Eye, Plus, Save, Settings2, UserPlus, Users } from 'lucide-react'
...
export function MimicaSetupScreen() {
  ...
  const canStart = mode === 'individual' ? count >= 2 : teamsReady && preparedChallengesReady
  return <div className="min-h-dvh pb-10"><Header backTo="/games/mimica" title="Configurar partida" /><section className="px-5 py-6">
    ...
    </section> : <section><TeamRosterFields activeButtonClassName="bg-violet-400 text-slate-950 hover:bg-violet-300" guestName={guestName} guests={guests} players={group?.players ?? []} selectedClassName="bg-violet-400/10 text-violet-300" selectedIds={selectedIds} setError={setError} setGuestName={setGuestName} setGuests={setGuests} setSelectedIds={setSelectedIds} setTeamName={setTeamName} setTeams={setTeams} teamIdPrefix="mimica-team" teamLabel="time" teamLabelPlural="equipes" teamName={teamName} teams={teams} onManagePlayers={() => navigate('/players')} />
