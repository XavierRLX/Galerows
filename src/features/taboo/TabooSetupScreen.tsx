...
export function TabooSetupScreen() {
  ...
  const begin = async () => {
    ...
    if (mode === 'teams' && !teamsReady) { setError('Monte pelo menos 2 times e coloque cada jogador em um deles.'); return }
  ...
  const canStart = mode === 'individual' ? count >= 2 : teamsReady
  return <div className="min-h-dvh pb-10"><Header backTo="/games/taboo" title="Configurar partida" /><section className="px-5 py-6">
    ...
    </section> : <TeamRosterFields activeButtonClassName="bg-emerald-400 text-slate-950 hover:bg-emerald-300" guestName={guestName} guests={guests} players={group?.players ?? []} selectedClassName="bg-emerald-400/10 text-emerald-300" selectedIds={selectedIds} setError={setError} setGuestName={setGuestName} setGuests={setGuests} setSelectedIds={setSelectedIds} setTeamName={setTeamName} setTeams={setTeams} teamIdPrefix="taboo-team" teamLabel="time" teamLabelPlural="times" teamName={teamName} teams={teams} onManagePlayers={() => navigate('/players')} />}
